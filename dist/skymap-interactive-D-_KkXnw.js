...
const us = {
  ...
  async searchHubble(n, s = 10) {
    const e = await fetch(
      `https://esahubble.org/images/?search=${encodeURIComponent(n)}`
    );
    if (!e.ok) throw new Error(`ESA Hubble API error: ${e.status}`);
    const t = (await e.text()).match(/var\s+images\s*=\s*(\[[\s\S]*?\]);/);
    if (!t) return [];
    let l;
    try {
      const r = t[1].replace(/([{,])\s*(\w+)\s*:/g, '$1"$2":').replace(/'/g, '"').replace(/,\s*([}\]])/g, "$1");
      l = JSON.parse(r);
    } catch {
      return [];
    }
    return l.slice(0, s).map((r) => {
      const g = r.id ?? "", o = g ? `https://cdn.esahubble.org/archives/images/large/${g}.jpg` : null, c = g ? `https://cdn.esahubble.org/archives/images/screen/${g}.jpg` : null;
      return {
        id: g,
        title: r.title ?? "",
        description: "",
...
const J = {
  ...
  m8: [{ filename: "Lagoon_Nebula_(ESO).jpg", credit: "ESO/INAF-VST/OmegaCAM" }],
  ...
  m20: [{ filename: "Trifid_3.6_8.0_24_microns_spitzer.png", credit: "NASA/JPL-Caltech" }],
  m27: [{ filename: "The_Dumbbell_Nebula_-_Messier_27.jpg", credit: "NASA · ESA" }],
  m31: [{ filename: "Andromeda_Galaxy_(with_h-alpha).jpg", credit: "Adam Evans (CC BY 2.0)" }],
  m33: [{ filename: "Messier33_-_HST_-_Heic1901a.jpg", credit: "NASA · ESA · M. Durbin, J. Dalcanton, B.F. Williams (Univ. of Washington)" }],
  ...
  m63: [{ filename: "Messier_63.jpg", credit: "ESA/Hubble · NASA" }],
  m78: [{ filename: "M78_(42903082152).jpg", credit: "ESA/Hubble · NASA/D. Padgett (GSFC)" }],
  ...
  m83: [{ filename: "Messier83_-_Heic1403a.jpg", credit: "NASA · ESA · Hubble Heritage Team" }],
  ...
  // ── Deep-sky extras ───────────────────────────────────────────────────────
  ngc7293: [{ filename: "Helix_Nebula.jpg", credit: "NASA · ESA · C.R. O'Dell (Vanderbilt)" }],
